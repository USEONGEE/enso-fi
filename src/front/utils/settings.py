...
class SettingMixin:
    ...
    @staticmethod
    def get_setting(context: ContextTypes.DEFAULT_TYPE) -> Self: ...
    @staticmethod
    def clear_setting(context: ContextTypes.DEFAULT_TYPE) -> None: ...
...
def setting_paths(root: str, key: str) -> Callable[[Type[T]], Type[T]]:
    ...
    def decorator(cls: Type[T]) -> Type[T]:
        def _get_setting(context: ContextTypes.DEFAULT_TYPE) -> T:
            return _get(context, root, key, cls)

        def _clear_setting(context: ContextTypes.DEFAULT_TYPE) -> None:
            _clear(context, root, key)

        # 정적 메서드로 주입
        cls.get_setting = staticmethod(_get_setting)  # type: ignore[attr-defined]
        cls.clear_setting = staticmethod(_clear_setting)  # type: ignore[attr-defined]
        # 선택: 클래스에 키를 달아두고 싶으면
